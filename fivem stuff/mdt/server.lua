local uptime = "00h 00m" --dont touch
local debug = true --touch if you want
...
RegisterNetEvent('mdt:unlinkServer')
AddEventHandler("mdt:unlinkServer", function()
local source = source
local steamid = GetPlayerIdentifier(source,0)
local realSteamID = tostring(tonumber(string.sub(steamid, 7), 16))
debug("unlinkServer - User: " .. source .. " Steam: " .. realSteamID .. " ")
...
RegisterNetEvent('mdt:showCharactersServer')
AddEventHandler("mdt:showCharactersServer", function()
local src = source
local steamid = GetPlayerIdentifier(source,0)
local realSteamID = tostring(tonumber(string.sub(steamid, 7), 16))
debug("showCharactersServer - User: " .. source .. " Steam: " .. realSteamID .. " ")
MySQL.Async.fetchAll("SELECT user_id FROM mdt_users WHERE steam=@steam", {['@steam'] = realSteamID}, function(users)
debug("showCharactersServer - user_id: " .. users[1].user_id .. " # of users: " .. #users)
if #users == 0 then
...
TriggerClientEvent('mdt:showCharacters', src, characters)
end)
end
end)
end)
...
--adds vehicle to db
RegisterNetEvent('mdt:registerVehicle')
AddEventHandler("mdt:registerVehicle", function(vehicleModel, vehiclePlate, vehicleColor, id)
local source = source
debug("registerVehicle - Model: " .. vehicleModel .. " Plate: " .. vehiclePlate .. " Color: " .. vehicleColor .. " id: " .. id)
MySQL.Async.fetchAll("SELECT * FROM mdt_characters WHERE id=@id", {['@id'] = id}, function(users)
debug("registerVehicle - lic: " .. users[1].lic)
...
--
RegisterNetEvent('mdt:me')
AddEventHandler("mdt:me", function(msg, name)
debug("me - msg: " .. msg .. " name: " .. name)
TriggerClientEvent('mdt:me', -1, source, msg, name)
end)

--displays chat message with character name instead of player name
RegisterNetEvent('mdt:sendMSG')
AddEventHandler("mdt:sendMSG", function(characterName, msg)
debug("sendMSG - source: " .. source .. " characterName: " .. characterName .. " msg: " .. msg)
...
RegisterNetEvent('mdt:showID')
AddEventHandler("mdt:showID", function(giveID, characterID, userid)
debug("showID - source: " .. source .. " giveID: " .. giveID .. " characterID: " .. characterID .. " userid: " .. userid)
MySQL.Async.fetchAll("SELECT * FROM mdt_characters WHERE id=@characterID", {['@characterID'] = characterID}, function(characters)
debug("showID - characters[1].last: " .. characters[1].last .. " characters[1].first: " .. characters[1].first .. " characters[1].dob: " .. characters[1].dob .. " characters[1].gender: " .. characters[1].gender)

TriggerClientEvent('mdt:showID', giveID, characters[1].last, characters[1].first, characters[1].dob, characters[1].gender, userid)
end)
end)

--triggers return id on client side
RegisterNetEvent('mdt:returnID')
AddEventHandler("mdt:returnID", function(name, id)
debug("returnID - source: " .. source .. " name: " .. name .. " id: " .. id)
TriggerClientEvent('mdt:returnID', id, name)
end)


function setCharacter(characterName,characterID) 
debug("setCharacter - source: " .. source .. " characterName: " .. characterName .. " characterID: " .. characterID)
...
debug("chatMessage - source: " .. source .. " color: " .. color .. " msg: " .. msg)
local src = source
CancelEvent()
TriggerClientEvent('mdt:getName', src, msg, 'mdt:sendMSG')
end)
...
debug("restart_server - source: " .. source .. " rawCommand: " .. rawCommand)
if (source == 0) then
...
debug("ban - source: " .. source .. " rawCommand: " .. rawCommand)
if source == 0 then
...
debug("kick - source: " .. source .. " rawCommand: " .. rawCommand)
if source == 0 then
local id = table.remove(args, 1)
local reason = table.concat(args, ' ')
...
debug("announce - source: " .. source .. " rawCommand: " .. rawCommand)
if source == 0 then
...
--sets server status
RegisterCommand("serverstatus", function(source, args, rawCommand)
debug("serverstatus - source: " .. source .. " rawCommand: " .. rawCommand)
if source == 0 then
local status = os.date("%Y-%m-%d %H:%M:%S")
MySQL.Sync.execute("UPDATE mdt_server SET status=@status, players=@players, uptime=@uptime WHERE id=@id", {['@status'] = status, ['@players'] = GetNumPlayerIndices(), ['@uptime'] = uptime, ['@id'] = args[1]})
end
end)
