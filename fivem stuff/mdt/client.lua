...
RegisterNetEvent("mdt:showCharacters")
AddEventHandler("mdt:showCharacters", function(c)
characters = c
GuiChange('character')
print(characters)
    SendNUIMessage({
...
RegisterCommand('changeCharacter', function(source, args)
        TriggerServerEvent('mdt:showCharactersServer')
end, false)
...
RegisterNUICallback(
    ...
    function(data)
		...
		characterID = data.id
		TriggerServerEvent('mdt:setCharacter', characterName, characterID)
...
TriggerServerEvent('mdt:registerVehicle', vehicleModel, vehiclePlate, vehicleColor, characterID)
end
else
...
--used in addons 
RegisterNetEvent("eventCharacterID")
AddEventHandler("eventCharacterID", function(type, event)
local event = event
if characterID == nil then
...
for i,array in pairs(event) do
event[i] = string.gsub(array, "%%characterID%%", characterID)
end

if type == "server" then
TriggerServerEvent(event[1], event[2], event[3], event[4], event[5], event[6], event[7], event[8], event[9], event[10])
else
TriggerEvent(event[1], event[2], event[3], event[4], event[5], event[6], event[7], event[8], event[9], event[10])
end
end)
...
RegisterCommand('giveID', function(source, args)
	...
	TriggerEvent('chatMessage', "^*^1ERRORS", { 128, 128, 128 }, "^rPlease specify the ID of the person to give to.")
	else
	TriggerEvent('chatMessage', "^*^3SYSTEM", { 128, 128, 128 }, "^rID given to " .. args[1])
	TriggerServerEvent('mdt:showID', args[1], characterID, GetPlayerServerId(PlayerId()))
...
TriggerServerEvent('mdt:me', msg, characterName)
end)

--sends everyone in the radius the /me message
RegisterNetEvent('mdt:me')
AddEventHandler("mdt:me", function(id, message, name)
print(message)
...
TriggerServerEvent('mdt:returnID', characterName, holdingID)
EnableGui(false)
holdingID = nil
...
AddEventHandler("mdt:returnID", function(name)
...
AddEventHandler("mdt:showID", function(last, first, dob, gender, id)
	holdingID = id
	print("id" .. first .. " " .. last)
	GuiChange('showID')
...
RegisterNetEvent('mdt:getName')
AddEventHandler("mdt:getName", function(msg, event)
if characterName == nil then
TriggerServerEvent('mdt:showCharactersServer')
else
...
RegisterCommand('link', function(source, args)
		TriggerServerEvent('mdt:linkServer', randomNumber())
end, false)

--unlink command (more in server)
RegisterCommand('unlink', function(source, args)
		TriggerServerEvent('mdt:unlinkServer')
end, false)
